...
pub fn uniquify_gdb(input: &str) -> Result<(), String> {
    let r_match_tid =
        Regex::new(r#"Thread\s+(?P<tid>\d+)\s+\(Thread 0x.*?\s+\(LWP\s+(?P<lwp>\d+\))"#).unwrap();
    if r_match_tid.captures(input).is_none() {
        return Err("not generated by gdb".to_owned());
    };

    let r_match_empty = Regex::new(r#"^$"#).unwrap();
    let r_match_entry = Regex::new(r#"\s*#\s*\d+\s+0x.+"#).unwrap();
